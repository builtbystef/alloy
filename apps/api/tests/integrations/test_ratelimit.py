...
from alloy_api.auth.router import LOGIN_PER_EMAIL, LOGIN_PER_IP
from alloy_api.config import Settings
from alloy_api.core.exceptions import AppError, RateLimitedError, handle_app_error
from alloy_api.integrations.ratelimit import (
    Hit,
    Limit,
    Limiter,
...
def test_limiter_refuses_past_the_limit_with_retry_after():
    ...
    async def scenario() -> None:
        ...
        await limiter.hit(TWO_PER_MINUTE, "s")
        with pytest.raises(RateLimitedError, match="Too many attempts") as info:
            await limiter.hit(TWO_PER_MINUTE, "s")
...
def test_limiter_check_does_not_count_and_reset_clears():
    ...
    async def scenario() -> None:
        ...
        await limiter.hit(TWO_PER_MINUTE, "s")
        await limiter.hit(TWO_PER_MINUTE, "s")
        with pytest.raises(RateLimitedError, match="Too many attempts"):
            await limiter.check(TWO_PER_MINUTE, "s")
