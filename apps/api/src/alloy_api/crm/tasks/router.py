...
from alloy_api.crm.pagination import PageOf, paginate
from alloy_api.crm.tasks import service
from alloy_api.crm.tasks.schemas import TaskCreate, TaskFilters, TaskRead, TaskUpdate
...
async def list_tasks(
    ...
    return await paginate(session, service.tasks_query(membership, filters), filters, TaskRead)
...
async def read_task(task_id: UUID, session: SessionDep, membership: CanReadCrm) -> TaskRead:
    return TaskRead.model_validate(await service.get_task(session, membership, task_id))
