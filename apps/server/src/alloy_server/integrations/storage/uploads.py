...
class UploadStore:
    ...
    async def upload_url(self, key: str, content_type: str, size: int) -> str:
        return await self.objects.upload_url(key, content_type, size, self.url_ttl)

    async def download_url(self, key: str, filename: str) -> str:
        return await self.objects.download_url(key, filename, self.url_ttl)
    ...
    async def verify(self, key: str) -> ObjectInfo:
        ...
        info = await self.objects.head(key)
        if info is None:
