import type { ChatUploadRead, ChatUploadTicket, ConversationRead } from "@alloy/api-client";

import { browserApi } from "@/lib/api/client";
import { unwrap } from "@/lib/api/errors";
import { contentTypeOf } from "@/lib/api/upload";

export async function createConversation(ws: string): Promise<ConversationRead> {
  return unwrap(
...
export async function completeChatUpload(ws: string, uploadId: string): Promise<ChatUploadRead> {
  return unwrap(
