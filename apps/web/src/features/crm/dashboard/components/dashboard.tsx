...
/** Read-only, so a Server Component is all it takes: no client bundle, no query cache. */
    ...
    <div className="flex flex-col gap-6">
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <StatCard
          ...
          href={`${paths.tasks}?due=today&status=open`}
        />
        <StatCard
          title="Overdue"
          value={dashboard.overdue_tasks}
          icon={<AlertTriangleIcon />}
          href={`${paths.tasks}?due=overdue&status=open`}
...
function StatCard({
  ...
  value: number;
  icon: ReactNode;
  href: WorkspacePaths["contacts"] | `${WorkspacePaths["tasks"]}?${string}`;
  alert?: boolean;
}) {
  return (
    <Link
            ...
            {contacts.map((contact) => (
              ...
              >
                <div className="min-w-0">
                  <Link href={paths.contact(contact.id)} className="font-medium hover:underline">
                    {contact.name}
                  </Link>
